...
# Try Ledoit–Wolf shrinkage (needs scikit-learn). Fall back to sample cov + ridge if not available.
try:
    Sigma = risk_models.CovarianceShrinkage(rets, returns_data=True).ledoit_wolf()
except Exception:
    Sigma = risk_models.sample_cov(rets, frequency=freq, returns_data=True)
    # light, scale-aware ridge to improve conditioning (no sklearn needed)
    lam = 1e-4 * np.trace(Sigma.values) / len(Sigma.columns)
    Sigma = pd.DataFrame(Sigma.values + lam * np.eye(len(Sigma)), index=Sigma.index, columns=Sigma.columns)
...
# Theoretical mean–variance with leverage cap:
...
# Optional faint reference ray using last two EF points (not the unconstrained CML)
if len(ef_vols) >= 2:
...
st.caption(
    "No sklearn required: falls back to sample covariance + ridge if Ledoit–Wolf shrinkage isn't available. "
    "Frontier is traced with the same γ optimization and leverage cap so the Γ point lies on it."
)
