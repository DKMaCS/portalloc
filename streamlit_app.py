...
# ---------------- Closed-form γ-portfolio for:
# min 0.5 w^T Σ w - (1/γ) μ^T w,  s.t. 1^T w = 1  (no box bounds)
# KKT -> w = (1/γ) Σ^{-1} μ - λ Σ^{-1} 1, with λ = ((1/γ) 1^T Σ^{-1} μ - 1)/(1^T Σ^{-1} 1)
def gamma_weights(mu_vec, Sigma_mat, gamma, ridge=0.0):
    ...
    invS_mu = np.linalg.solve(Sigma_mat, mu_vec)     # Σ^{-1} μ
    invS_1  = np.linalg.solve(Sigma_mat, ones)       # Σ^{-1} 1
    A = float(ones @ invS_1)                         # 1^T Σ^{-1} 1
    B = float(ones @ invS_mu)                        # 1^T Σ^{-1} μ
    lam = ((1.0/gamma) * B - 1.0) / A
    w = (1.0/gamma) * invS_mu - lam * invS_1
    return w

# ---------------- Trace EF by sweeping γ ----------------
gammas = np.logspace(np.log10(g_max), np.log10(g_min), n_frontier)  # high→low risk aversion
ef_rets, ef_vols, ef_ws = [], [], []
last_w = None
for g in gammas:
    w = clean(gamma_weights(mu_vec, Sigma_mat, g))
...
# ---------------- Selected γ portfolio (lies on the curve) ----------------
w_gamma = clean(gamma_weights(mu_vec, Sigma_mat, gamma_sel))
...
st.caption(
    "Frontier generated by minimizing ½ wᵀΣw − (1/γ) μᵀw under ∑w=1 across a log-spaced γ grid. "
    "Because the Γ-portfolio uses the same formulation, it lies on the plotted frontier."
)
