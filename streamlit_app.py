...
with st.sidebar:
    ...
    # γ sweep range (log space) — covers very risk-averse to risk-seeking
    g_low  = st.number_input("γ min (log sweep)", value=1e-2, format="%.4g")
    g_high = st.number_input("γ max (log sweep)", value=1e4,  format="%.4g")
...
def clean_weights_array_from_dict(dct, tickers, cutoff=1e-8):
    ...
    return clean_weights_array(w, cutoff=cutoff)
...
def solve_gamma_portfolio(mu_vec, Sigma_mat, rf, gamma):
    ...
    for solver in [cp.OSQP, cp.SCS, cp.ECOS]:
        try:
            ...
            pass
    raise RuntimeError("CVXPY failed for γ-portfolio (unconstrained).")

def compute_frontier_by_gamma(mu_vec, Sigma_mat, rf, g_min, g_max, n_points):
    """Trace the efficient frontier by sweeping γ (same objective/feasible set)."""
    ...
    for g in gammas:
        ...
        # skip near-duplicates (same active set)
        if last_w is not None and np.linalg.norm(w - last_w, 1) <= 1e-8:
            continue
        r, s, _ = portfolio_stats(w, mu_vec, Sigma_mat, rf)
        rets.append(r); vols.append(s); weights.append(w); last_w = w
    return np.array(vols), np.array(rets), weights, gammas[:len(weights)]

# ---------------- Key portfolios ----------------
# γ portfolio (this one must lie on the frontier we trace)
w_gamma = solve_gamma_portfolio(mu_vec, Sigma_mat, rf, gamma)
w_gamma = clean_weights_array(w_gamma)
gamma_ret, gamma_vol, gamma_sharpe = portfolio_stats(w_gamma, mu_vec, Sigma_mat, rf)

# GMV / Tangency for reference (unconstrained)
# These don't affect the frontier; just markers.
...
w_gmv = clean_weights_array_from_dict(w_gmv_dict, tickers, cutoff=0.0)
...
w_tan = clean_weights_array_from_dict(w_tan_dict, tickers, cutoff=0.0)
tan_ret, tan_vol, tan_sharpe = ef_tan.portfolio_performance(risk_free_rate=rf)

# Frontier curve traced by γ
ef_vols, ef_rets, ef_weights, ef_gammas = compute_frontier_by_gamma(
    mu_vec, Sigma_mat, rf, g_low, g_high, n_frontier
)
...
if ef_vols.size:
    ...
    ax.plot(ef_vols[order], ef_rets[order], lw=1.6, alpha=0.95, label="Efficient Frontier (γ-sweep)")
...
# CML through tangency (optional reference)
...
ax.set_title("Efficient Frontier — Unconstrained (∑w=1), traced by γ")
...
st.caption(
    "Frontier is traced by solving the γ-objective across a log-spaced γ grid. "
