...
with st.sidebar:
    ...
    ridge = st.number_input("Ridge on Σ (stability)", value=1e-8, step=1e-8, format="%.0e")
...
def compute_frontier_by_gamma(mu_vec, Sigma_mat, rf, lb, ub, n_points, dedup_eps=1e-6):
    gammas = np.logspace(np.log10(G_SWEEP_MAX), np.log10(G_SWEEP_MIN), n_points)
    vols, rets, ws = [], [], []
    seen = set()
    for g in gammas:
        ...
        if key in seen:
            continue
        seen.add(key)
        vols.append(s); rets.append(r); ws.append(w)
    vols = np.array(vols); rets = np.array(rets)
    order = np.argsort(vols)
    return vols[order], rets[order], [ws[i] for i in order]

# Key portfolios
w_gamma = solve_gamma_portfolio(mu_vec, Sigma, rf, gamma, lower_bound, upper_bound)
gamma_ret, gamma_vol, gamma_sharpe = portfolio_stats(w_gamma, mu_vec, Sigma, rf)

w_gmv = solve_gmv(Sigma, lower_bound, upper_bound)
gmv_ret, gmv_vol, _ = portfolio_stats(w_gmv, mu_vec, Sigma, rf)

ef_vols, ef_rets, ef_ws = compute_frontier_by_gamma(
    mu_vec, Sigma, rf, lower_bound, upper_bound, n_frontier
)

# Constrained tangency from the same EF
if ef_vols.size:
...
st.markdown("### Weights (non-zero only)")
def weights_df(weights, tickers, thresh=1e-8):
    df = pd.DataFrame({"Ticker": tickers, "Weight": weights})
    df["Weight"] = df["Weight"].round(6)
    return df[df["Weight"].abs() > thresh].sort_values("Weight", ascending=False).reset_index(drop=True)
...
st.caption(
    "Frontier generated by sweeping γ (fixed log-range) under identical box bounds. "
    "Kinks correspond to active-set/boundary changes (expected under box constraints)."
)
